...
st.set_page_config(page_title="조립공정 실적", layout="centered")

st.markdown(
    '<div style="background:linear-gradient(135deg,#85B7EB 0%,#185FA5 100%);'
    'border-radius:10px;padding:1.25rem 1.5rem;margin-bottom:1rem;">'
    '<div style="font-size:22px;font-weight:600;color:#fff;">조립공정 실적</div>'
    '<div style="font-size:13px;color:rgba(255,255,255,0.85);margin-top:2px;">'
    "노칭 · 스태킹 · 파우치 실링 · 전해액 주입 — 스태킹에서 셀(CELL)이 처음 탄생합니다.</div></div>",
...
with st.container(border=True):
    st.markdown("##### 공정 선택")
    processes = queries.processes_by_group("조립")
...
# ── 데이터 조회 ──────────────────────────────────────────────
group_stats = queries.production_group_stats("조립")
today_count = queries.today_group_lot_count("조립", datetime.now().strftime("%Y-%m-%d"))
...
with st.container(border=True):
    ...
    if not work_orders:
        ...
        st.warning(
            f"'{process_name}' 공정에 투입할 수 있는(잔여수량 있는) 재공품이 없습니다."
        )
    elif available_lots_df.empty:
        st.warning(f"'{process_name}' 공정에 투입할 수 있는(잔여수량 있는) LOT이 없습니다.")
    else:
        ...
        if "assembly_start_dt" not in st.session_state:
            st.session_state["assembly_start_dt"] = datetime.now()

        c4, c5 = st.columns([1, 1])
        start_clock = c4.time_input(
            "시작시각",
            value=st.session_state["assembly_start_dt"].time().replace(second=0, microsecond=0),
            key="assembly_start_clock",
        )
        ...
        if recipe:
            ...
            output_items = queries.assembly_output_items()
            output_item_options = {f"{r['item_code']} | {r['item_name']}": r for r in output_items}
            output_item_label = st.selectbox("결과 품목", list(output_item_options.keys()))
            selected_output_item = output_item_options[output_item_label]
