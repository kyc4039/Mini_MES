...
from src.services import (
    InputLotUsage,
    InspectionEntry,
    ProcessProductionRegistration,
    register_inspection_results,
    register_process_production,
)


st.set_page_config(page_title="화성공정 실적", layout="centered")

st.markdown(
    '<div style="background:linear-gradient(135deg,#F09595 0%,#A32D2D 100%);'
    'border-radius:10px;padding:1.25rem 1.5rem;margin-bottom:1rem;">'
    '<div style="font-size:22px;font-weight:600;color:#fff;">화성공정 실적</div>'
    '<div style="font-size:13px;color:rgba(255,255,255,0.85);margin-top:2px;">'
    "에이징 · 화성 충방전 · 디개싱 · 최종검사 — 검사기준 대비 측정값을 입력하면 자동으로 PASS/FAIL이 판정됩니다.</div></div>",
...
with st.container(border=True):
    st.markdown("##### 공정 선택")
    processes = queries.processes_by_group("화성")
...
# ── 데이터 조회 ──────────────────────────────────────────────
group_stats = queries.production_group_stats("화성")
today_count = queries.today_group_lot_count("화성", datetime.now().strftime("%Y-%m-%d"))
work_orders = queries.work_orders_in_progress()
all_available_lots_df = queries.available_lots(("RAW", "WIP", "CELL"))
...
with kpi_cols[2]:
    metric_card("투입가능 LOT", f"{len(available_lots_df)}")
with kpi_cols[3]:
...
with st.container(border=True):
    ...
    if not work_orders:
        ...
        st.warning(f"'{process_name}' 공정에 투입할 수 있는(잔여수량 있는) 재공품이 없습니다.")
    else:
        ...
        if "fusion_start_dt" not in st.session_state:
            st.session_state["fusion_start_dt"] = datetime.now()

        c4, c5 = st.columns([1, 1])
        start_clock = c4.time_input(
            "시작시각",
            value=st.session_state["fusion_start_dt"].time().replace(second=0, microsecond=0),
            key="fusion_start_clock",
        )
        ...
        if recipe:
            output_items = queries.assembly_output_items()
            matched = [r for r in output_items if pr.matches_prefix(r["item_code"], recipe["output_prefix"])]
            if len(matched) == 1:
                selected_output_item = matched[0]
                st.info(f"결과 품목: {selected_output_item['item_code']} | {selected_output_item['item_name']} ({selected_output_item['item_type']})")
            elif len(matched) > 1:
                ...
                st.error(f"'{recipe['output_prefix']}' 계열의 결과 품목을 찾을 수 없습니다.")
                selected_output_item = None
        else:
            selected_output_item = None

        if selected_output_item:
            if selected_output_item["item_type"] == "CELL":
                process_tag = pr.PROCESS_ENGLISH_TAG.get(selected_process["process_code"], selected_process["process_code"])
                suggested_lot_no = pr.suggest_cell_lot_no(start_dt, process_tag)
            else:
                existing_lot_nos = queries.lot_numbers_for_item(selected_output_item["item_id"])
                suggested_lot_no = f"{selected_output_item['item_code']}-LOT001" if not existing_lot_nos else existing_lot_nos[0]
            ...
            if st.button("생산실적 저장", type="primary", disabled=not start_time_valid):
                ...
                try:
                    ...
                    st.session_state["fusion_last_lot_no"] = result["output_lot_no"]
                    st.rerun()
...
with st.container(border=True):
    ...
    specs = queries.inspection_specs_by_process(selected_process["process_id"])
    if not specs:
        st.caption(f"'{process_name}' 공정에는 등록된 검사기준이 없습니다.")
    else:
        recent_lots = queries.lots_by_process(selected_process["process_id"])
        recent_lot_options = {row["lot_no"]: row["lot_id"] for row in recent_lots}

        if not recent_lot_options:
            st.info("이 공정에서 생성된 LOT이 아직 없습니다. 먼저 위에서 생산실적을 등록하세요.")
        else:
            default_index = 0
            last_lot_no = st.session_state.get("fusion_last_lot_no")
            if last_lot_no in recent_lot_options:
                default_index = list(recent_lot_options.keys()).index(last_lot_no)

            target_lot_label = st.selectbox(
                "검사 대상 LOT", list(recent_lot_options.keys()), index=default_index
            )
            target_lot_id = recent_lot_options[target_lot_label]

            measured_values: dict[int, float] = {}
            for spec in specs:
                limit_text = f"기준 {spec['lower_limit']}~{spec['upper_limit']} {spec['unit']}"
                measured_values[spec["spec_id"]] = st.number_input(
                    f"{spec['spec_name']} ({limit_text})",
                    value=0.0, step=0.01,
                    key=f"insp_{selected_process['process_id']}_{spec['spec_id']}",
                )

            inspected_clock = st.time_input(
                "검사 시각", value=datetime.now().time().replace(second=0, microsecond=0), key="fusion_inspect_clock"
            )

            if st.button("검사결과 저장"):
                inspected_dt = datetime.combine(datetime.now().date(), inspected_clock)
                entries = [
                    InspectionEntry(spec_id=spec_id, measured_value=value)
                    for spec_id, value in measured_values.items()
                ]
                try:
                    results = register_inspection_results(
                        target_lot_id, entries, inspected_dt.strftime("%Y-%m-%d %H:%M")
                    )
                    for r in results:
                        icon = "✅" if r["judge"] == "PASS" else "❌"
                        st.write(f"{icon} {r['spec_name']}: {r['measured_value']} → **{r['judge']}**")
                    st.rerun()
                except ValueError as exc:
                    st.error(str(exc))

            st.markdown("**이 LOT의 검사 이력**")
            insp_history_df = queries.inspection_results_for_lot(target_lot_id)
            if insp_history_df.empty:
                st.caption("검사 이력이 없습니다.")
            else:
                rows_html = ""
                for row in insp_history_df.itertuples():
